...
<form class="form-horizontal" action="{site_url}/transaksi/utang/save" method="POST">
	<div class="box">
		...
		<div class="box-body">
			...
			<div class="form-group">
				...
				<div class="col-sm-6">
					<select name="proyek" id="proyek" class="form-control select2">
						...
						<? foreach(get_all_where('mproyek', array('tipe' => 1) ) as $u): ?>
							<option value="<?= $u->no ?>" <?= ($u->no == $proyek) ? 'selected' : '' ?> ><?= $u->nama.' ('.get_by_id('mrekanan', array('no' => $u->rekanan) )->nama.')' ?></option>								
						<? endforeach ?>
					</select>
				</div>
			</div>
			...
			<div class="form-group">
				<label for="sisautang" class="col-sm-3 control-label">Sisa Utang</label>
				<div class="col-sm-6">
					<div class="input-group">
						<div class="input-group-addon"><i class="fa fa-lock"></i></div>
						<input readonly type="text" class="form-control" id="sisautang" name="sisautang" placeholder="0" value="{sisautang}">
					</div>
				</div>
			</div>
			<div class="form-group">
...
<script type="text/javascript">
	$(document).on('change','#proyek', function(){
		var n = $(this).val();
		$.getJSON("{site_url}/transaksi/utang/total_saldo/"+n, function(data){
			$('#sisautang').val(data.sisautang)
	...
	$(document).on('keyup','#dibayar', function(){
		...
		$.getJSON("{site_url}/transaksi/utang/total_saldo/"+$('#proyek').val(), function(data){
			var sisautang = parseFloat(data.sisautang)
			var terbayar = parseFloat(data.terbayar)
			if(n > sisautang) {
				alert('nominal terlalu besar')
				$('#dibayar').val(0)
				$('#terbayar').val(terbayar)
				$('#sisautang').val(sisautang)
			} else {
				var totalterbayar = n+terbayar;
				if($('#terbayar').val(totalterbayar)) {					
					$('#sisautang').val(data.utang - totalterbayar)
				}
			}
		})
